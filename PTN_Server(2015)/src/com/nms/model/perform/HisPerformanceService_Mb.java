...
import com.nms.corba.ninterface.impl.performance.CorbaPerformance;
...
public class HisPerformanceService_Mb extends ObjectService_Mybatis {
	...
	public int saveOrUpdate(HisPerformanceInfo hisInfo) throws Exception {
		...
		try {
			if (hisInfo.getId() != 0) {
				resultcesId = this.historyPerformanceMapper.updateByPrimaryKey(hisInfo);
			} else {
				resultcesId = this.historyPerformanceMapper.insert(hisInfo);
			}
			sqlSession.commit();
		} catch (Exception e) {
...
//					}
	...
	public void insertAll(List<HisPerformanceInfo> hisPerformance) {
		for (int i = 0; i < hisPerformance.size(); i++) {
			historyPerformanceMapper.insert(hisPerformance.get(i));
		}
		sqlSession.commit();
	}
	...
	public List<HisPerformanceInfo> selectByTime(String startTime,
		...
		try {
			if(taskIdList != null && taskIdList.size()>0){
				hisInfoList = historyPerformanceMapper.queryByTime(startTime, endTime, taskIdList,siteId);
			}else{
				hisInfoList = historyPerformanceMapper.queryByTime(startTime, endTime, null,siteId);
			}
	 ...
	 * @throws Exception
	 */
	public List<HisPerformanceInfo> queryByCorbaPerfor(CorbaPerformance info)throws Exception{
		List<HisPerformanceInfo> hisList =null;
		try{
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Map<String, Object> map = new HashMap<String, Object>();
			if(info.getSiteId()>0){
				map.put("siteId", info.getSiteId());
			}
			if(UiUtil.isNull(info.getPortName())){
				map.put("portName", DButil.toSingleMark(info.getPortName()));
			}
			if(info.getLayerList()!=null&&info.getLayerList().size()==1){
				map.put("monitor", info.getLayerList().get(0));
			}
			map.put("startTime", sdf.format(new Date(info.getStartTime())));
			map.put("endTime", sdf.format(new Date(info.getEndTime())));
			
			hisList=this.historyPerformanceMapper.queryByConditionMap(map);
			wrapHisPerformanceInfo(hisList,null);
		}catch(Exception e){
			throw e;
		}
		return hisList;
