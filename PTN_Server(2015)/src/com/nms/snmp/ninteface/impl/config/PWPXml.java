...
public class PWPXml {
	...
	public String getPWPXml() {
		//CMCC-PTN-NRM-ME-V1.0.0-20140411-1602-P00.xml
		String filePath = "";
		String version = ResourceUtil.srcStr(StringKeysLbl.LBL_SNMPMODEL_VERSION);
		String[] xmlPath = {"snmpData\\ZJ\\CS\\EB\\OMC\\CM\\"+DateUtil.getDate("yyyyMMdd"), "CM-PTN-PWP-A1-"+version+"-"+XmlUtil.getTime()+".xml"};
		...
		try {
		    ...
		    XmlUtil.createFile(doc, "CM-PTN-PWP-A1-",filePath);
		} catch (Exception e){
	...
	private void createXML(Document doc,List<PwInfo> pwList){
		...
		root.appendChild(XmlUtil.fileHeader(doc,"PWPW"));
	...
	private Element createFileContent(Document doc,List<PwInfo> pwList) {
		...
		this.createElementNode(doc, "N", "number", FieldName, "i", "3");
