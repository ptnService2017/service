...
public class PRTTestXml {
	...
	public String getPRTXml() {
		...
		String time = DateUtil.getDate("yyyyMMdd");
		String time2 = XmlUtil.getTime();
		creatFiles(time, time2, "-001");
		creatFiles(time, time2, "-002");
		return "";
	}
	
	private void creatFiles(String time,String time2,String index){
		String filePath = "";
		String version = ResourceUtil.srcStr(StringKeysLbl.LBL_SNMPMODEL_VERSION);
		String[] xmlPath = {"snmpData\\ZJ\\CS\\EB\\OMC\\CM\\"+time, "CM-PTN-PRT-A1-"+version+"-"+time2+index+".xml"};
