 ...
 *
 */
public class AnalysisAlarmReportTable extends AnalysisBase{
...
//	}
	...
	public AlarmObject analysisCommandToObject(byte[] commands) throws Exception{
		String configXml1 = "com/nms/drive/analysis/xmltool/file/报告告警配置.xml";
		String configXml2 = "com/nms/drive/analysis/xmltool/file/报告告警单盘配置.xml";
		String configXml3 = "com/nms/drive/analysis/xmltool/file/报告告警单盘配置_SUB.xml";
		String configXml4 = "com/nms/drive/analysis/xmltool/file/报告告警单盘配置_SUB_SUB.xml";
...
//			}
	...
	private AlarmLineObject getAlarmLineObject(String fileName, byte[] lineObjectBuffer) throws Exception {
		...
		try {
			...
			line[0] = 0;
			line[1] = 0;
			line[2] = lineObjectBuffer[0];
			line[3] = lineObjectBuffer[1];
			alarmLineObject.setLine(CoderUtils.bytesToInt(line));
			//线路数据长度
			byte[] lineCount = new byte[4];
			lineCount[0] = lineObjectBuffer[2];
			lineCount[1] = lineObjectBuffer[3];
			lineCount[2] = lineObjectBuffer[4];
			lineCount[3] = lineObjectBuffer[5];
			alarmLineObject.setLineInfoCount(CoderUtils.bytesToInt(lineCount));
...
//			}
	...
	private AlarmInfoObject getAlarmInfoObject(String fileName, byte[] alarmInfoBuffer) throws Exception {
		...
		int alarmCode = CoderUtils.bytesToInt(alarmInfoBuffer[0]);
		int alarmStatus = CoderUtils.bytesToInt(alarmInfoBuffer[1]);
		byte[] yearArr = new byte[4];
		yearArr[0] = 0;
		yearArr[1] = 0;
		yearArr[2] = alarmInfoBuffer[2];
		yearArr[3] = alarmInfoBuffer[3];
		int year = CoderUtils.bytesToInt(yearArr);
		int month = CoderUtils.bytesToInt(alarmInfoBuffer[4]);
		int day = CoderUtils.bytesToInt(alarmInfoBuffer[5]);
		int hour = CoderUtils.bytesToInt(alarmInfoBuffer[6]);
		int min = CoderUtils.bytesToInt(alarmInfoBuffer[7]);
		int second = CoderUtils.bytesToInt(alarmInfoBuffer[8]);
		alarmInfoObject.setAlarmCode(alarmCode);
		alarmInfoObject.setAlarmStatus(alarmStatus);
		alarmInfoObject.setAlarmDate(year+"-"+month+"-"+day+" "+hour+":"+min+":"+second);
//		AlarmInfoObject alarmInfoObject = new AlarmInfoObject();
//		try {
//			AnalysisCommandByDriveXml analysisCommandByDriveXml = new AnalysisCommandByDriveXml();
