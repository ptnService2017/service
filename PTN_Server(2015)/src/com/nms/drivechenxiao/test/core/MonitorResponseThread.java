	...
	private void cfml() {
		if (command != null && command.length > 8) {
			while (true) {
				...
				System.out.print("byte[] tt = new byte[]{");
				for (int i = 0; i < oneCommand.length; i++) {
					System.out.print(oneCommand[i] + ",");
				}
				System.out.println("};");

//								AnalysisXC analysis = new AnalysisXC();
//								analysis.analysisXc(oneCommand, null);
//	AnalysisRING ar = new AnalysisRING();	
//	ar.analysisRing(oneCommand, null);
//				oneCommand
				AnalysisInformation analysisInformation = new AnalysisInformation();
				hzPtnMainUI.getJTextArea1().setText(hzPtnMainUI.getJTextArea1().getText() + "--------------------------------------------\r\n" + CoreOper.print16String(oneCommand) + "\r\n" + analysisInformation.AnalysisResponds(oneCommand) + "\r\n");
				hzPtnMainUI.tbgd();
				hzPtnMainUI.zdgd();
